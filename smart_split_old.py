...
def detect_photo_regions(image: np.ndarray, min_area_ratio: float = 0.02, max_area_ratio: float = 0.30) -> List[PhotoRegion]:
    """
    Detect photo regions in the scanned image using hierarchical contours.
    
    Args:
        image: Input BGR image
        min_area_ratio: Minimum area ratio (relative to image size) for a valid photo
        max_area_ratio: Maximum area ratio (relative to image size) for a valid photo
    ...
    # Find contours with hierarchy to detect inner rectangles
    contours, hierarchy = cv2.findContours(
        processed, 
        cv2.RETR_TREE,  # Get hierarchy
    ...
    for i, contour in enumerate(contours):
        ...
        # Filter by area (between 2% and 30% of image)
        ...
        if width > 0 and height > 0:
            ...
            if aspect_ratio > 3:  # Photos should be roughly square or slightly rectangular
                continue
